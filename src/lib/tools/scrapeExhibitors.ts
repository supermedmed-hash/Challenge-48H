...
async function analyzePageStructure(page: Page, userQuery?: string): Promise<PageSelectors> {
...
2. Les fiches exposants ont souvent des URLs avec /exhibitor/, /exhibitors/, /exposant/, /exposants/, /company/, /stand/, /participant/, /brand/, /marque/ ou un ID numérique.
...
6. PRÉFÈRE LES SÉLECTEURS SIMPLES (ex: 'a.item-link') aux chemins longs (ex: 'div.grid > ul > li > a').

RECHERCHE DE L'UTILISATEUR (CONTEXTE):
${userQuery || 'Aucune requête spécifique'}`,
  })) as any;
...
async function resolveBestSelector(page: Page, selectors: PageSelectors): Promise<string> {
  ...
  for (const sel of candidates) {
    ...
    // 2. Si le sélecteur est complexe (contient > ou plusieurs espaces), on essaie de le simplifier
    // Garder seulement la dernière partie (ex: "ul > li > a.link" -> "a.link")
    if (sel.includes(' ') || sel.includes('>')) {
      const parts = sel.split(/[ >]+/).filter(Boolean);
      const simpler = parts[parts.length - 1];
      if (simpler && simpler !== 'a' && simpler !== 'div' && simpler !== 'li') {
        try {
          const count = await page.evaluate((s: string) => {
            try { return document.querySelectorAll(s).length; } catch { return 0; }
          }, simpler);
          if (count > 0) {
            console.log(`[Selector] 🔄 Simplification réussie: "${sel}" -> "${simpler}" (${count} éléments)`);
            return simpler;
          }
        } catch { }
      }
  ...
  const heuristicSelector = await page.evaluate(() => {
    const patterns = ['/exhibitor', '/exhibitors', '/exposant', '/exposants', '/company', '/stand', '/participant', '/brand', '/marque'];
    ...
    if (matches.length > 3) {
      ...
      return 'a[href*="/exhibitor"], a[href*="/exhibitors"], a[href*="/exposant"], a[href*="/company"], a[href*="/stand"]';
    }
    return null;
  });

  return heuristicSelector || 'a[href*="/exhibitor"], a[href*="/exhibitors"], a[href*="/exposant"], a[href*="/company"]';
}

/**
 * Résolution du sélecteur "Suivant" avec fallbacks communs
 */
async function resolveNextSelector(page: Page, initialSelector: string): Promise<string | null> {
  ...
  const handler = (request: any) => {
    ...
    if (
      (url.includes('/api/') || url.includes('/wp-json/') || url.includes('/graphql') ||
        ...
        url.includes('/exhibitor') || url.includes('page=') || url.includes('per_page=') || 
        url.includes('offset=') || url.includes('cursor=') || url.includes('_page=')) &&
...
async function* collectViaApi(
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    if ('error' in responseData) {
      yield { type: 'status', message: `API erreur ${responseData.error} à l'itération ${i + 1}` };
      break;
    }
...
async function* collectExhibitorLinksUniversal(
  ...
  const linkPattern = /\/appearances\/|\/exhibitors?\/|\/exposants?\/|\/company\/|\/startup\//;
  ...
  if (apiCandidates.length > 0) {
    ...
    yield { type: 'status', message: `API(s) non productives, basculement en mode DOM...` };
  } else {
    yield { type: 'status', message: `Pas d'API détectée, mode DOM activé` };
  }
  ...
  const extractAllLinksFromHtml = async (): Promise<number> => {
    const extracted = await page.evaluate((pattern: string) => {
      ...
      const urlRegex = /href="([^"]*(?:\/appearances\/|\/exhibitors?\/|\/exposants?\/|\/company\/|\/startup\/)[^"]*)"/g;
  ...
  if (selectors.navType === 'pagination') {
    while (pageNum <= maxPages) {
      ...
      if (domCount === 0 && pageNum > 1) {
        yield { type: 'status', message: `Page ${pageNum} — sélecteur voit 0 éléments, re-résolution...` };
        const newSelector = await resolveBestSelector(page, selectors);
        if (newSelector !== activeSelector) {
          activeSelector = newSelector;
          yield { type: 'status', message: `Nouveau sélecteur: "${activeSelector}"` };
        }
      }

      await extractAllLinksFromHtml();
      const newFound = allLinks.size - countBefore;
      yield { type: 'status', message: `Page ${pageNum} — DOM: ${domCount} éléments | +${newFound} nouveaux (total: ${allLinks.size})` };
      ...
      // Résolution du bouton suivant (peut changer entre les pages)
      const nextBtnSelector = await resolveNextSelector(page, selectors.nextSelector);
      ...
      if (!clicked) {
        // Fallback: Tentative de navigation par URL si le bouton est absent
        yield { type: 'status', message: `Bouton "suivant" introuvable, tentative par URL...` };
        
        const currentUrl = page.url();
        let nextUrl = '';
        
        if (currentUrl.includes('page=')) {
          nextUrl = currentUrl.replace(/page=(\d+)/, (_, p) => `page=${parseInt(p) + 1}`);
        } else if (currentUrl.includes('?')) {
          nextUrl = `${currentUrl}&page=${pageNum + 1}`;
        } else {
          nextUrl = `${currentUrl}?page=${pageNum + 1}`;
        }

        if (nextUrl !== currentUrl) {
          yield { type: 'status', message: `Navigation forcée: Page ${pageNum + 1}` };
      ...
      if (!clicked) {
        yield { type: 'status', message: `Échec de navigation — fin de pagination` };
        break;
      }

      const domChanged = await waitForMoreElements(page, activeSelector, currentCount, 10000);
      if (!domChanged) {
        // Pour les sites à chargement AJAX total sans changement de count, on attend un peu
    ...
    while (pageNum <= maxPages) {
      ...
      const countBefore = allLinks.size;
      await extractAllLinksFromHtml();
      yield { type: 'status', message: `Chargement ${pageNum} — total: ${allLinks.size} liens` };
    ...
    while (pageNum <= maxPages) {
      ...
      yield {
        type: 'status',
        message: `Scroll ${pageNum} — +${newLinks} nouveaux | total: ${allLinks.size} | pos: ${Math.round(scrollY / scrollHeight * 100)}%`
      };
...
export async function* scrapeExhibitorsStream(url: string, userQuery?: string): AsyncGenerator<ScrapeProgressEvent> {
  ...
  try {
    ...
    const selectors = await analyzePageStructure(page, userQuery);
    yield {
      type: 'status',
    ...
    const total = Math.min(links.length, 100);
    ...
    for (let i = 0; i < total; i += batchSize) {
      ...
      if (errorStreak >= 3) {
        ...
        yield { type: 'status', message: `Trop d'erreurs, réduction de la parallélisation (batch: ${batchSize})` };
